...
#include "inc/2738.inc"
#include "inc/3739.inc"
#include "inc/3740.inc"
#include "inc/4275b.inc"
#include "inc/4276b.inc"
#include "inc/bolzen.inc"
#include "inc/lenkrad.inc"

/* control center----------------------------------------------------------- */

declare fast = 0;           // 1=less details quick, 0=all details slow
declare use_area=0;         // 0=off, 1=on
declare matrix_mode = 0;    // enter the matrix
...
#macro material_reflect(COLOR)
  texture {
    pigment { COLOR }
    finish { ambient 0.4 phong 1 reflection 0.025 }
    normal { granite .02 scale .01 }
  }
#end
...
#if (fast=0)
  text { ttf "inc/Generic.ttf" "LEGO" 0.07, 0
    scale <-0.15, -0.15, -0.2>
    rotate x*90
...
#declare rhdr=360*rand(r_l);
  ...
  sphere {
    0,500
    texture {
      pigment{image_map{hdr "inc/pd_kitchen_probe_1" map_type 1 interpolate 2}}
...
//    rgb (White+SkyBlue+Blue*.1)*400000*lk
  ...
  sphere{0,300
    pigment{
      image_map{hdr "inc/jvp_studio-light-to-hdr-5" map_type 1 interpolate 2}
      rotate 90*y
    }
  ...
  plane { <0,1,0>, 0
    texture{
      pigment{
        image_map{jpeg "inc/square_wooden_floor_tiles_3030650.jpg" interpolate 2}
      }
      normal{
        bump_map{jpeg "inc/square_wooden_floor_tiles_3030650.jpg" interpolate 2 bump_size 1}
      }
      finish{reflection{0.01,.045}}
      rotate 90*x
    scale 30
    }
  }
